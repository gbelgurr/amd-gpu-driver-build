...
if test x$1 = x32; then
    ...
    cd build32
    cmake ../llvm -G Ninja \
        -DCMAKE_INSTALL_PREFIX=$prefix/llvm-i386 -DLLVM_ENABLE_PROJECTS="clang;libclc" -DLLVM_ENABLE_DUMP=ON \
    ...
    cmake ../llvm -G Ninja \
        -DCMAKE_INSTALL_PREFIX=$prefix/llvm -DLLVM_ENABLE_PROJECTS="clang;libclc" -DLLVM_ENABLE_DUMP=ON \
        -DLLVM_TARGETS_TO_BUILD="X86;AMDGPU" -DLLVM_ENABLE_ASSERTIONS=ON \
        -DCMAKE_BUILD_TYPE=Release -DLLVM_BUILD_LLVM_DYLIB=ON -DLLVM_LINK_LLVM_DYLIB=ON -DLLVM_APPEND_VC_REV=OFF \
